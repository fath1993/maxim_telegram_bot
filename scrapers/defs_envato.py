...
def envato_check_if_sign_in_is_needed(envato_account):
    ...
    while True:
        try:
            while True:
                custom_log("envato_check_if_sign_in_is_needed: starting page load", f"scrapers")
                ...
                try:
                    ...
                    custom_log(f"envato_check_if_sign_in_is_needed: {str(e)}", f"scrapers")
                    envato_account.envato_account_description = f'envato cookie does not exist. please provide new cookie'
                    envato_account.is_account_active = False
                    envato_account.save()
                    return True
                ...
                if current_url != 'https://elements.envato.com/sign-in':
                    message = f'auth with *{envato_account.envato_cookie.path}* cookie has been accepted.'
                    custom_log(message, f"scrapers")
                    envato_account.envato_account_description = message
                    envato_account.is_account_active = True
                    envato_account.save()
                    driver.quit()
                    return False
                else:
                    message = f'auth with *{envato_account.envato_cookie.path}* cookie has been failed. it means new sign-in require'
                    custom_log(message, f"scrapers")
                    envato_account.envato_account_description = message
                    envato_account.is_account_active = False
                    envato_account.save()
            ...
            custom_log('envato_check_if_sign_in_is_needed webdriver exception. err: ' + str(e), f"scrapers")
        ...
        if webdriver_problem_number_of_reloading == 3:
            message = "envato_check_if_sign_in_is_needed: webdriver exception caused auth function to be aborted"
            custom_log(message, f"scrapers")
            envato_account.envato_account_description = message
            envato_account.save()
            driver.quit()
...
def envato_download_file(envato_file, account_to_use=None):
    ...
    options.add_argument("--disable-setuid-sandbox")
    options.add_argument("--headless=new")
    envato_files_path = BASE_DIR / 'media/cr/'
    ...
    if len(direct_download_list) != 0:
        ...
        custom_log("envato_download_file: start downloading the files", f"scrapers")
        number_of_try = 0
        while True:
            try:
                custom_log(f"envato_download_file: start downloading the file: {direct_download_file.unique_code}", f"scrapers")
                ...
                custom_log(f"envato_download_file: failed to download file: {direct_download_file.unique_code} try/except-> err: " + str(
                               e), f"scrapers")
                time.sleep(1)
                number_of_try += 1
            if number_of_try == 3:
                custom_log(f"envato_download_file: after 3 time of reloads failed to download file: {direct_download_file}", f"scrapers")
    ...
    if len(scrap_then_download_list) != 0:
        ...
        while True:
            try:
                ...
                while True:
                    try:
                        custom_log("envato_download_file: starting page load", f"scrapers")
                        ...
                        # time.sleep(3600)
                        custom_log("driver.get(url)> url has been fetched. we are waiting for: " + str(
                                       get_time_sleep()), f"scrapers")
                        # time.sleep(get_time_sleep())
                        time.sleep(1)
                        check_chrome_connection_status(driver)
                        custom_log("envato_download_file: start downloading the files", f"scrapers")
                        ...
                        i = 0
                        try:
                            custom_log("envato_download_file: start downloading the file: " + str(
                                           scrap_then_download_file), f"scrapers")
                            driver.switch_to.new_window(f'tab_{i}')
                            driver.get(scrap_then_download_file.page_link)
                            time.sleep(5)
                            custom_log("envato_download_file: tab title is: " + str(driver.title), f"scrapers")
                            number_of_checking_categories = 0
                            while_situation = True
                            while True:
                                try:
                                    custom_log("envato_download_file: check that if categories are visible", f"scrapers")
                                    WebDriverWait(driver, 15).until(
                                        EC.presence_of_all_elements_located(
                                            (By.CSS_SELECTOR, '.mIe6goMR.igMtSiA8.dp4yRDc1')))
                                    custom_log("envato_download_file: categories are visible", f"scrapers")
                                    categories_section = 'NOT 3D'
                                    custom_log("envato_download_file: category is NOT 3D", f"scrapers")
                                    break
                                except Exception as e:
                                    custom_log("envato_download_file: failed to find none 3D category", f"scrapers")
                                    try:
                                        custom_log("envato_download_file: check for 404 error page", f"scrapers")
                                        WebDriverWait(driver, 20).until(
                                            EC.visibility_of_element_located((By.CLASS_NAME, 'DsS8cj_E')))
                                        custom_log("envato_download_file: 404 error page has showed up", f"scrapers")
                                ...
                                if number_of_checking_categories == 2:
                                    ...
                                    custom_log("envato_download_file: category is 3D", f"scrapers")
                                    break
                            if not while_situation:
                                custom_log("envato_download_file: 404 error page has happened for: " + str(
                                               scrap_then_download_file), f"scrapers")
                            ...
                            if categories_section == '3D':
                                ...
                                while True:
                                    try:
                                        custom_log("envato_download_file: check for psd btn", f"scrapers")
                                        WebDriverWait(driver, 15).until(
                                            EC.presence_of_element_located((By.CLASS_NAME, 'GK9O2uz3')))
                                        custom_log("envato_download_file: psd btn has showed up", f"scrapers")
                                        break
                                    except Exception as e:
                                        try:
                                            ...
                                            if str(download_limit_warning).find('Fair Use Warning') != -1:
                                                ...
                                                scrap_then_download_file.in_progress = False
                                                scrap_then_download_file.save()
                                                custom_log(f"fair usage policy. for more information visit elements.envato.com. time: {jdatetime.datetime.now().strftime('%Y/%m/%d %H:%M')}", f"scrapers")
                                                return
                                        except:
                                            pass
                                        custom_log("envato_download_file: failed to find psd btn", f"scrapers")
                                        number_of_try_to_pick_psd_btn += 1
                                        time.sleep(5)
                                    if number_of_try_to_pick_psd_btn == 3:
                                        driver.quit()
                                        custom_log("envato_download_file: after 3 times of reload, failed to find psd btn", f"scrapers")
                                        while_situation = False
                                        break
                                if not while_situation:
                                ...
                                psd_btn.click()
                                custom_log("envato_download_file: psd_btn has been clicked", f"scrapers")
                                time.sleep(1)
                                ...
                                while True:
                                    try:
                                        custom_log("envato_download_file: check for download_this_angle_btn", f"scrapers")
                                        WebDriverWait(driver, 60).until(
                                            EC.element_to_be_clickable((By.CLASS_NAME, 'mIe6goMR')))
                                        custom_log("envato_download_file: download_this_angle_btn has showed up", f"scrapers")
                                        break
                                    except Exception as e:
                                        custom_log("envato_download_file: failed to find download_this_angle_btn", f"scrapers")
                                        number_of_try_to_pick_download_this_angle_btn += 1
                                        time.sleep(5)
                                    if number_of_try_to_pick_download_this_angle_btn == 3:
                                        driver.quit()
                                        custom_log("envato_download_file: after 3 times of reload, failed to find download_this_angle_btn", f"scrapers")
                                        while_situation = False
                                        break
                                if not while_situation:
                                    custom_log("envato_download_file: failed to download file: " + str(
                                ...
                                custom_log("envato_download_file: download_this_angle_btn has been clicked", f"scrapers")
                                time.sleep(1)
                                ...
                                while True:
                                    try:
                                        custom_log("envato_download_file: check for download_without_license_btn", f"scrapers")
                                        WebDriverWait(driver, 60).until(
                                            EC.element_to_be_clickable((By.CLASS_NAME, 'l0Q2UI85')))
                                        custom_log("envato_download_file: download_without_license_btn has showed up", f"scrapers")
                                        break
                                    except Exception as e:
                                        custom_log("envato_download_file: failed to find download_without_license_btn", f"scrapers", )
                                        number_of_try_to_pick_download_without_license_btn += 1
                                        time.sleep(5)
                                    if number_of_try_to_pick_download_without_license_btn == 3:
                                        driver.quit()
                                        custom_log("envato_download_file: after 3 times of reload, failed to find download_without_license_btn", f"scrapers")
                                        while_situation = False
                                        break
                                if not while_situation:
                                    custom_log("envato_download_file: failed to download file: " + str(
                                ...
                                custom_log("envato_download_file: download_without_license_btn has been clicked", f"scrapers")
                                time.sleep(1)
                            else:
                                ...
                                while True:
                                    try:
                                        custom_log("envato_download_file: check for download btn", f"scrapers")
                                        WebDriverWait(driver, 60).until(
                                            EC.element_to_be_clickable((By.CLASS_NAME, 'igMtSiA8')))
                                        custom_log("envato_download_file: download btn has showed up", f"scrapers")
                                        break
                                    except Exception as e:
                                        custom_log("envato_download_file: failed to find download btn", f"scrapers")
                                        number_of_checking_download_btn_visibility += 1
                                        time.sleep(5)
                                    if number_of_checking_download_btn_visibility == 3:
                                        driver.quit()
                                        custom_log("envato_download_file: after 3 times of reload, failed to find download btn", f"scrapers")
                                        while_situation = False
                                        break
                                if not while_situation:
                                    custom_log("envato_download_file: failed to download file: " + str(
                                ...
                                download_btn.click()
                                time.sleep(1)
                                custom_log("envato_download_file: download_btn has been clicked", f"scrapers")

                                number_of_checking_download_without_license_btn_visibility = 0
                                while_situation = True
                                while True:
                                    try:
                                        custom_log("envato_download_file: check for download without license btn", f"scrapers")
                                        WebDriverWait(driver, 60).until(
                                            EC.element_to_be_clickable((By.CLASS_NAME, 'l0Q2UI85')))
                                        custom_log("envato_download_file: without license btn btn has showed up", f"scrapers")
                                        break
                                    except Exception as e:
                                        try:
                                            download_limit_warning = driver.page_source
                                            ...
                                            pass
                                        custom_log("envato_download_file: failed to find without license btn btn", f"scrapers")
                                    ...
                                    if number_of_checking_download_without_license_btn_visibility == 3:
                                        driver.quit()
                                        custom_log("envato_download_file: after 3 times of reload, failed to find without license btn btn", f"scrapers")
                                        while_situation = False
                                        break
                                if not while_situation:
                                    custom_log("envato_download_file: failed to download file: " + str(
                            ...
                            while True:
                                try:
                                    custom_log("envato_download_file: check for chrome downloads page", f"scrapers")
                                    WebDriverWait(driver, 15).until(
                                        EC.visibility_of_element_located((By.XPATH,
                                                                          '/html/body/downloads-manager')))
                                    custom_log("envato_download_file: chrome downloads has showed up", f"scrapers")
                                    break
                                except Exception as e:
                                    custom_log("envato_download_file: failed to find chrome downloads page. err: " + str(
                                                   e), f"scrapers")
                                    chrome_downloads_page_visibility += 1
                                if chrome_downloads_page_visibility == 3:
                                    driver.quit()
                                    custom_log("envato_download_file: after 3 times of reload, failed chrome downloads page", f"scrapers")
                                    while_situation = False
                                    break
                            if not while_situation:
                                custom_log("envato_download_file: failed to download file: " + str(
                            ...
                            scrap_then_download_file.in_progress = False
                            scrap_then_download_file.save()
                        custom_log("envato_download_file: finish a list.", f"scrapers")
                        return True
                    except Exception as e:
                        number_of_get_accept_cookie_btn_tries += 1
                        if number_of_get_accept_cookie_btn_tries == 3:
                            custom_log("envato_download_file: after 3 times of reload, download has been failed. err: " + str(
                                           e), f"scrapers")
                            driver.quit()
                            scrap_then_download_file.download_percentage = 0
                            scrap_then_download_file.is_acceptable_file = True
                ...
                custom_log('envato_download_file webdriver exception. err: ' + str(e), f"scrapers")
                custom_log('we are waiting for ' + str(get_time_sleep()) + ' second', f"scrapers")
                time.sleep(get_time_sleep())
            except WebDriverException as e:
                custom_log('envato_download_file webdriver exception. err: ' + str(e), f"scrapers")
            ...
            if webdriver_problem_number_of_reloading == 3:
                driver.quit()
                custom_log("envato_download_file: webdriver exception caused download to be aborted", f"scrapers")
                scrap_then_download_file.download_percentage = 0
                scrap_then_download_file.is_acceptable_file = True
                scrap_then_download_file.in_progress = False
                scrap_then_download_file.save()
