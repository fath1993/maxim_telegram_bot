...
from motionarray.models import get_motion_array_config_settings, MotionArrayAccount
...
def check_if_sign_in_is_needed(motion_array_account):
    options = Options()
    ...
    while True:
        try:
            ...
            while True:
                ...
                try:
                    ...
                    motion_array_account.motion_array_account_description = f'cookie does not exist. please provide new cookie'
                ...
                try:
                    ...
                    try:
                        ...
                        motion_array_account.motion_array_account_description = str(plan_summery.text)
                        motion_array_account.save()
                        driver.quit()
                        ...
                        motion_array_account.motion_array_account_description = f'account plan not confirmed'
                        motion_array_account.save()
                        return True
                except Exception as e:
                    custom_log(
                ...
                if number_of_get_accept_cookie_btn_tries == 2:
                    ...
                    custom_log(
                        ...
                        "d")
                    motion_array_account.motion_array_account_description = f'after 2 times of reload,  auth function with cookie has been failed. it means new sign-in require'
                    motion_array_account.save()
                    driver.quit()
                    return True
        ...
        if webdriver_problem_number_of_reloading == 3:
            ...
            motion_array_account.motion_array_account_description = f'webdriver exception caused auth function to be aborted'
            motion_array_account.save()
            return True
...
class MotionArrayCheckAuthThread(threading.Thread):
    ...
    def run(self):
        ...
        for motion_array_account in motion_array_accounts:
            check_if_sign_in_is_needed(motion_array_account)


def motion_array_download_file(motion_array_files, account_to_use):
    options = Options()
    options.add_argument("--window-size=1920,1200")
    options.add_argument("--no-sandbox")
    options.add_argument("--disable-dev-shm-usage")
    ...
    for motion_array_file in motion_array_files:
...
def download_file_with_aria2_download_manager(motion_array_file, file_name: str, url: str):
    motion_array_file.download_percentage = 0
    motion_array_file.save()
    time.sleep(1)
    aria2 = aria2p.API(
        aria2p.Client(
            host="http://localhost",
    ...
    while not download.is_complete:
        try:
            ...
            if download.status == 'error':
                download.pause()
                motion_array_file.download_percentage = int(download.progress)
                motion_array_file.is_acceptable_file = True
                motion_array_file.in_progress = False
                motion_array_file.save()
            ...
            if int(percentage) != 100:
                motion_array_file.download_percentage = int(percentage)
                motion_array_file.save()
                time.sleep(1)
        except Exception as e:
            attempt += 1
            ...
            if attempt == 10:
                ...
                motion_array_file.is_acceptable_file = True
                motion_array_file.in_progress = False
                motion_array_file.file_storage_link = None
                motion_array_file.save()
                return False
    if download.is_complete:
        try:
            motion_array_file.download_percentage = 100
            motion_array_file.is_acceptable_file = True
            motion_array_file.in_progress = False
            motion_array_file.file.name = f'/envato/files/{file_name}'
            motion_array_file.save()
            if str(url).find('storagebox.de') == -1:
                UploadManager(motion_array_file).start()
            custom_log(
                f'download_file_with_aria2_download_manager. result: download file: {motion_array_file.unique_code} has been finished successfully')
            return True
        except Exception as e:
            custom_log(f'download_file_with_aria2_download_manager. try/except:> err: {str(e)}')
            return False
